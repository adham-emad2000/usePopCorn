    ...
    <div>
      ...
      <Main>
        <Box>
          ...
          {!isLoading && !isError && (
            <MovieList movies={movies} onSelectHandler={handleSelectedMovie} />
          )}
        </Box>
...
function MovieList({ movies, onSelectHandler }) {
  return (
...
function Movie({ movie, onSelectHandler }) {
  return (
    <li onClick={() => onSelectHandler(movie.imdbID)}>
...
function MoviesDetails({ selectedId, onCloseMovie, AddtoWatch, watched }) {
  ...
  useEffect(
    function () {
      async function getMovieDetails() {
        ...
        setMovie(data); // Store the fetched movie details in state
        SetIsLoading(false);
      }
      getMovieDetails();
    },
          ...
          <section>
            <div className="rating">
              {!isWatched ? (
                ...
                <p>This movie was rated {WatchedUserRating}</p>
              )}
            </div>
  ...
  return (
    <div className="summary">
      ...
      <div>
        <p>
          <span>#️⃣</span>
          <span className="max_width">{watched.length} movies</span>
        </p>
        <p>
          <span>⭐️</span>
        ...
        <p>
          <span>⏳</span>
          <span className="max_width">{avgRuntime.toFixed(1)} min</span>
        </p>
      </div>
    </div>
  );
